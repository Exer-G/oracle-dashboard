...
console.log('[Tracker] Starting...', isElectronApp ? '(Electron Desktop)' : '(Web)');
...
async function signInWithGoogle() {
    try {
        if (isElectronApp) {
            // Electron: Open OAuth in a child window via IPC
            // First, get the OAuth URL from Supabase without redirecting
            const { data, error } = await supabaseClient.auth.signInWithOAuth({
                provider: 'google',
                options: {
                    redirectTo: 'https://oracle-tracker.netlify.app/',
            ...
            // Open OAuth URL in Electron child window
            const result = await window.electronAPI.openOAuth(data.url);
            ...
            if (result && result.accessToken) {
                ...
                console.log('[Auth] Electron OAuth successful');
            } else {
    ...
    try {
        if (supabaseClient) {
            ...
            if (data && data.length > 0) {
                // Add Supabase projects that aren't already in TT_PROJECTS
...
function setupTimerCallbacks() {
    ...
    engine.onTick = (data) => {
        ...
        // Update system tray (Electron only, every 5 seconds to reduce IPC calls)
        if (isElectronApp && data.totalElapsed % 5 === 0) {
            window.electronAPI.updateTrayState({
                state: 'running',
                elapsed: formatTime(data.totalElapsed)
            });
        }
    };
    ...
    engine.onStateChange = (state) => {
        ...
        // Update system tray state (Electron only)
        if (isElectronApp) {
            window.electronAPI.updateTrayState({
                state: state,
                elapsed: state === 'running' ? formatTime(engine.getElapsedSeconds()) : null
            });
        }
    };
}
...
async function toggleTimer() {
    ...
    if (engine.state === 'running') {
        ...
        // Request screen capture permission (auto-grants in Electron)
        const hasCapture = await window.screenshotCapture.requestPermission();
        if (!hasCapture && !isElectronApp) {
            toast('Screen capture declined. Timer will run without screenshots.', 'error');
        }

        // Start systems (activity start is async in Electron for IPC)
        await window.activityTracker.start();
...
async function syncProjectsAndAllocations() {
    ...
    if (statusEl) statusEl.innerHTML = '<span style="color:var(--warning);">Syncing...</span>';

    let synced = 0;

    try {
        // 1. Sync projects from Supabase projects table
        const { data: supaProjects, error: projErr } = await supabaseClient
            .from('projects')
            .select('id, name, status, hourly_rate, source, description')
            .order('name');

        if (!projErr && supaProjects && supaProjects.length > 0) {
            // Merge with local TT_PROJECTS
            const existingNames = new Set(TT_PROJECTS.map(p => p.name.toLowerCase()));
            const newProjects = supaProjects.filter(p => !existingNames.has(p.name.toLowerCase()));

            mergedProjects = [...TT_PROJECTS, ...newProjects.map(p => ({
                id: p.id, name: p.name, client: '', status: p.status || 'active'
            }))];

            await populateProjectDropdowns();
            synced++;
            console.log('[Sync] Loaded', supaProjects.length, 'projects from Supabase (' + newProjects.length + ' new)');
        }

        // 2. Sync team members from tt_team_members
        const { data: supaTeam, error: teamErr } = await supabaseClient
            .from('tt_team_members')
            .select('*')
            .order('name');
        ...
        if (!teamErr && supaTeam && supaTeam.length > 0) {
            ...
            synced++;
            console.log('[Sync] Loaded', supaTeam.length, 'team members');
        }
        ...
        if (currentUser) {
            ...
            if (settingsData?.preferences?.allocations) {
                ...
                synced++;
                console.log('[Sync] Loaded', settingsData.preferences.allocations.length, 'allocations');
            }
        }

        localStorage.setItem('tt_last_sync', new Date().toISOString());
        updateLastSyncDisplay();

        if (statusEl) statusEl.innerHTML = '<span style="color:var(--success);">Synced ' + synced + ' data sources</span>';
        toast('Synced projects, team & allocations', 'success');

    } catch (err) {
        console.error('[Sync] Error:', err);
        if (statusEl) statusEl.innerHTML = '<span style="color:var(--danger);">Sync failed: ' + err.message + '</span>';
...
function checkForUpdates() {
    ...
    const updateUrl = 'https://oracle-tracker.netlify.app/updates/latest.json';

    const statusEl = document.getElementById('syncStatus');
    if (statusEl) statusEl.innerHTML = '<span style="color:var(--warning);">Checking for updates...</span>';

    fetch(updateUrl)
        .then(r => {
            if (!r.ok) throw new Error('No update info available');
            return r.json();
        })
        .then(info => {
            if (info.version && info.version !== currentVersion) {
                if (statusEl) statusEl.innerHTML =
                    '<div style="padding:12px;background:var(--grey-100);border-radius:8px;">' +
                    '<strong>Update available: v' + info.version + '</strong><br>' +
                    '<span style="font-size:12px;color:var(--text-secondary);">' + (info.notes || '') + '</span><br>' +
                    '<a href="' + (info.downloadUrl || 'https://oracle-tracker.netlify.app') + '" target="_blank" ' +
                    'style="color:var(--primary);font-size:13px;margin-top:4px;display:inline-block;">Download update</a>' +
                    '</div>';
                toast('Update available: v' + info.version, 'success');
            } else {
                if (statusEl) statusEl.innerHTML = '<span style="color:var(--success);">You are on the latest version (v' + currentVersion + ')</span>';
                toast('You are on the latest version', 'success');
            }
        })
        .catch(() => {
            // No update server yet — just show current version
            if (statusEl) statusEl.innerHTML = '<span style="color:var(--success);">Current version: v' + currentVersion + ' (latest)</span>';
            toast('You are on the latest version', 'success');
        });
}

function updateLastSyncDisplay() {
    const lastSync = localStorage.getItem('tt_last_sync');
